                    ...
                    if ($result->num_rows > 0) {
                        ...
                        if ($voterToken == $storedVoterToken) {
                            ...
                            if ($storedVoterStatus == "no_vote") {
                                ...
                                if($storedVoterJenisVote == "offline") {
                                    $_SESSION['voter'] = $row['voter_id'];
                                    header("Location: /");
                                    exit();
                                } elseif ($storedVoterJenisVote == "online") {
                                    if (!isset($_SESSION['login_gagal'])) {
                                        $_SESSION['login_gagal'] = [
                                            'icon' => 'error',
                                            'title' => 'Login Gagal',
                                            'text' => "Anda tidak di izinkan melakukan voting secara offline, silahkan vote secara online",
                                        ];
                                    }
                                }
                            } else {
